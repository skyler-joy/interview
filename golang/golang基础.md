### go语言中make和new的区别是什么
1.  make是初始化内置的数据结构，初始化类型为slice、map、channel的数据，new的作用是根据传入的类型分配一片内存空间并返回指向这片内存空间的指针
2.  new 分配返回的是指针，即类型 *Type；make 返回引用，即 Type。

### 数组和切片的区别
1.  切片是指针类型，数组是值类型
2.  数组的赋值形式为值传递，切片的赋值形式为引用传递
3.  数组的长度是固定的，不能使用append追加数据。而切片长度可以任意调整（切片是动态的数组）
4.  切片底层是都引用了一个底层数组，切片本身不能存储任何数据，都是这底层数组存储数据，所以修改切片的时候修改的是底层数组中的数据。切片一旦扩容，指向一个新的底层数组，内存地址也就随之改变
...
rune相当int32，golang中的字符串底层实现是通过byte数组的，中文字符在unicode下占2个字节，在utf-8编码下占3个字节，而golang默认编码正好是utf-8，byte等同于int8，常用来处理ascii字符，rune 等同于int32,常用来处理unicode或utf-8字符
...
答：每个 defer 语句都对应一个\_defer 实例，多个实例使用指针连接起来形成一个单连表，保存在 gotoutine 数据结构中，每次插入\_defer 实例，均插入到链表的头部，函数结束再一次从头部取出，从而形成后进先出的效果。
...
map的类型是map\[key]，key类型的必须是可比较的，通常情况，会选择内建的基本类型，比如整数、字符串做key的类型。如果要使用struct作为key，要保证struct对象在逻辑上是不可变的。在Go语言中，map\[key]函数返回结果可以是一个值，也可以是两个值。map是无序的，如果我们想要保证遍历map时元素有序，可以使用辅助的数据结构，例如orderedmap。
...
![](https://pic2.zhimg.com/v2-1107961e741b834eb5fc071ff68da831_b.webp?consumer=ZHI_MENG)

**方式一、使用内置sync.Map，详细参考**

[https://mbd.baidu.com/ma/s/7Hwd9yMc](https://link.zhihu.com/?target=https%3A//mbd.baidu.com/ma/s/7Hwd9yMc)

**方式二、使用读写锁实现并发安全map**

[https://mbd.baidu.com/ma/s/qO7b0VQU](https://link.zhihu.com/?target=https%3A//mbd.baidu.com/ma/s/qO7b0VQU)
...
![](https://pic4.zhimg.com/v2-f99ed061ef6ea995743cda6d332861e3_b.webp?consumer=ZHI_MENG)
...
[golang 哪些类型可以作为map key](https://link.zhihu.com/?target=https%3A//blog.csdn.net/lanyang123456/article/details/123765745)
...
[go context详解 - 卷毛狒狒 - 博客园](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/juanmaofeifei/p/14439957.html)

Context被定义为一个接口类型，它包含了四个方法：
...
![](https://pic2.zhimg.com/v2-e287f5861392d5de73d8956974acdbbd_b.webp?consumer=ZHI_MENG)

就着图片里面的答案看看吧。

### **2、go channel 的底层实现原理 （数据结构）**

![](https://pic4.zhimg.com/v2-9452c08ff058590cea1b40a39fd8c70f_b.webp?consumer=ZHI_MENG)

底层结构需要描述出来，这个简单，buf，发送队列，接收队列，lock。

### **3、nil、关闭的 channel、有数据的 channel，再进行读、写、关闭会怎么样？（各类变种题型，重要）**

![](https://pic1.zhimg.com/v2-8e12b7eb664280346ec96bcc4fe1db2c_b.webp?consumer=ZHI_MENG)
...
![](https://pic2.zhimg.com/v2-d2a3f2aa5c85b94df49430c21a53a2d9_b.webp?consumer=ZHI_MENG)

**接收流程：**

![](https://pic3.zhimg.com/v2-69e0e2f1409a17ec2987e4b63e6210da_b.webp?consumer=ZHI_MENG)
...
[https://mbd.baidu.com/ma/s/ZMOKQATr](https://link.zhihu.com/?target=https%3A//mbd.baidu.com/ma/s/ZMOKQATr)
...
[Golang学习篇--协程池\_Word哥的博客-CSDN博客\_golang协程池](https://link.zhihu.com/?target=https%3A//blog.csdn.net/finghting321/article/details/106492915/)
...
[Golang GC算法解读\_suchy\_sz的博客-CSDN博客\_go的gc算法](https://link.zhihu.com/?target=https%3A//blog.csdn.net/shudaqi2010/article/details/90025192)
...
5）在本轮 GC 已经基本完成后，会调用 mProf\_PostSweep 方法。以此记录最后一次标记终止时的堆配置文件快照。
...
[php pcntl用法-PHP问题-PHP中文网](https://link.zhihu.com/?target=https%3A//www.php.cn/php-ask-473095.html)
